...
$(function() {
	$("#f").submit(function() {
		...
		if ($(":input[name=title]").val().trim() == "") {
			$(":input[name=title]").parents('.form-group').addClass('has-error').find().filter('span').show();
			alert("내용을 입력하세요");
			hasError = true;
		}
		if ($(":input[name=contents]").val().trim() == "") {
			$(":input[name=contents]").parents('.form-group').addClass('has-error').find().filter('span').show();
			alert("내용을 입력하세요");
			hasError = true;
		}
		return !hasError;
